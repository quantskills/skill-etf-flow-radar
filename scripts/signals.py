...
def s7_consec_flow(
    ...
    if not universe:
        return empty_hits()
    df = flow_df[flow_df["symbol"].isin(universe)].copy().sort_values(["symbol", "date"])
    K = int(consec_days)
    rows: list[dict] = []
    for symbol, g in df.groupby("symbol", sort=False):
        g = g[g["date"] <= date]
        if g.empty or g.iloc[-1]["date"] != date or len(g) < K + lookback:
            continue
        window = g.tail(K)
        ...
        rows.append(
            _hit_row(
                trade_date=date, symbol=symbol, signal_type="S7", signal_value=ratio,
                net_redemption_T=float(t_row["net_redemption"]),
                size_T=float(t_row["size"]) if "size" in t_row and pd.notna(t_row["size"]) else None,
                discount_rate_T=None,
